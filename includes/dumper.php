        ...
        foreach ($tables as $table)
        {
            ...
            while(($result = mysql_query ("SELECT * FROM {$table} LIMIT {$from}, {$limit}")) && ($total = mysql_num_rows($result)))
            {
                while($row = mysql_fetch_row($result))
                {
                    ...
                    $insert .= "INSERT INTO `{$table}` VALUES(" . implode(", ", $row) . ");\n";
                }
        ...
        while (($str = $this->fn_read_str($fp)) !== false)
        {
            ...
            if ($sql)
            {
                ...
                if ($execute)
                {
                    ...
                    mysql_query ($sql) or trigger_error ("Incorrected SQL query .<BR>" . mysql_error(), E_USER_ERROR);
