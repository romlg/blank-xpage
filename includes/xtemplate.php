...
2002-2005 Jeremy Coates [cocomp@users.sourceforge.net]
*/
...
class XTemplate
{
    ...
    /**
     * assign array of variables to block
     *
     * @access public
     * @param string $bname - Name of block
     * @param array $datas - associative array of values
     */
    function assign_array ($bname, $datas, $extra = "")
    {
        if (is_array ($datas))
        {
            $parse = false;
            foreach ($datas as $k => $v)
            {
                if (is_array ($v)) {
                    if (!is_numeric ($k))
                        $this->assign_array ($bname.".".$k, $v, $extra);
                    else
                        $this->assign_array ($bname, $v, $extra);
                }
                else {
                    $parse = true;
                    $this->vars[$k] = $v;
